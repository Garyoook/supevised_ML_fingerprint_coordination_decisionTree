...
def find_split(training_dataset):
    ...
    return {'left_split': left_split, 'right_split': right_split}, cur_split_point
...
def decision_tree_learning(training_dataset, depth):
    ...
    if isUniTag:
        ...
        (l_branch, l_depth), split_point = decision_tree_learning(l_dataset, depth + 1)
        (r_branch, r_depth), split_point = decision_tree_learning(r_dataset, depth + 1)
        node = {'attribute': 'wifi_signal > ', 'value': split[1], 'left': l_branch, 'right': r_branch, 'leaf': False}
